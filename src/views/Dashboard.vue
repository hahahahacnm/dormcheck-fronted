<template>
  <main class="p-10 min-h-[70vh] max-w-3xl mx-auto">
    <!-- 未登录提示 -->
    ...
    <section
      v-else
      class="space-y-8 p-6 rounded-lg bg-white/70 backdrop-blur-sm"
    >
      <h1 class="text-3xl font-extrabold text-blue-700 flex items-center gap-2">
        <svg class="w-7 h-7 text-blue-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="space-y-3 text-gray-700 text-lg">
        ...
        <p>
          ...
          <span :class="{'text-red-600 font-bold': user.role === 2,'text-gray-800': user.role !== 2}">
            {{ roleText }}
          </span>
        </p>
      </div>
      ...
      <div class="flex justify-end gap-4 flex-wrap">
        <button @click="onChangePassword" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">修改密码</button>
        <button @click="onChangeEmail" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">修改邮箱</button>
        <button @click="onSponsor" class="px-6 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 transition">赞助入口</button>
      </div>
    </section>
...
const user = reactive({ username: '', email: '', role: -1 })
...
function resetUser() { user.username = ''; user.email = ''; user.role = -1 }
...
@media (max-width: 640px) {
  ...
  .flex.justify-end { flex-wrap: wrap; gap: 0.75rem; }
  .flex.justify-end > button { flex: 1 1 auto; min-width: 120px; }
}
</style>
